...
DATA_URL = "https://gitlab.com/Dimbreath/AnimeGameData2/-/raw/main/{FOLDER}/{value}"
LANG_URL = "https://gitlab.com/Dimbreath/AnimeGameData2/-/raw/main/{LANG_FOLDER}/TextMap{key}.json"
_PATH = None
...
async def creat_artifact_sets():
    ...
    for key in _DATA["ARTIFACTS_SETS"]:
        data[key["id"]] = {
            "affixId": key["affixId"],
            "nameTextMapHash": key["nameTextMapHash"],
        }
...
async def creat_weapon():
    ...
    for key in _DATA["WEAPONS"]:
        data[key["id"]] = {
            "nameTextMapHash": key["nameTextMapHash"],
            "icon": key["icon"],
            "awakenIcon": key["awakenIcon"],
            "rankLevel": key["rankLevel"],
        }
...
async def creat_artifact():
    ...
    for key in _DATA["ARTIFACTS"]:
        data[key["id"]] = {
            ...
            "icon": key["icon"],
            "rankLevel": key["rankLevel"],
            "mainPropDepotId": key["mainPropDepotId"],
            "appendPropDepotId": key["appendPropDepotId"],
        }
...
async def creat_costume():
    ...
    for key in _DATA["COSTUME"]:
        data[key["skinId"]] = {
            "iconName": key["frontIconName"],
            "sideIconName": key["sideIconName"],
            "nameTextMapHash": key["nameTextMapHash"],
        }
...
async def creat_skill():
    ...
    for key in _DATA["SKILLS"]:
        if key["skillIcon"]:
            data[key["id"]] = {
                "nameTextMapHash": key["nameTextMapHash"],
                "skillIcon": key["skillIcon"],
                "proudSkillGroupId": key.get("proudSkillGroupId", ""),
            }
...
async def creat_skill_depo():
    ...
    for key in _DATA["SKILLDEPOT"]:
        if key["id"] == 101 or key.get("energySkill", 0) != 0:
            data[key["id"]] = {
                "id": key["id"],
                "energySkill": key["energySkill"],
                "skills": key["skills"],
                "subSkills": key["subSkills"],
                "talents": key["talents"],
            }
...
async def creat_characrer():
    ...
    for key in _DATA["SKILLS"]:
        skill_data[key["id"]] = {
            ...
            "skillIcon": key["skillIcon"],
            "proudSkillGroupId": key.get("proudSkillGroupId", ""),
            "costElemType": key.get("costElemType", None),
            "forceCanDoSkill": key.get("forceCanDoSkill", None),
        }

    for key in _DATA["AVATAR"]:
        if (
            key["skillDepotId"] == 101
            or key["iconName"].endswith("_Kate")
            or str(key["id"])[:2] == "11"
        ):
            continue
        ...
        if key["iconName"].endswith("_PlayerBoy") or key["iconName"].endswith(
            "_PlayerGirl"
        ):
            new_data = {
                "nameTextMapHash": key["nameTextMapHash"],
            ...
            for skill_id in key["candSkillDepotIds"]:
                ...
                if depot and depot["id"] != 101:
                    ...
                    new_data.update({"costElemType": element_id.get("costElemType")})
...
async def creat_lang(lang):
    ...
    for name in data:
        for key in _DATA_LANG[name]:
            data[name][key] = {
                "CHS": _DATA.get("CHS", {}).get(key, ""),
                "CHT": _DATA.get("CHT", {}).get(key, ""),
                "DE": _DATA.get("DE", {}).get(key, ""),
                "EN": _DATA.get("EN", {}).get(key, ""),
                "ES": _DATA.get("ES", {}).get(key, ""),
                "FR": _DATA.get("FR", {}).get(key, ""),
                "ID": _DATA.get("ID", {}).get(key, ""),
                "IT": _DATA.get("IT", {}).get(key, ""),
                "JP": _DATA.get("JP", {}).get(key, ""),
                "KR": _DATA.get("KR", {}).get(key, ""),
                "PT": _DATA.get("PT", {}).get(key, ""),
                "RU": _DATA.get("RU", {}).get(key, ""),
                "TH": _DATA.get("TH", {}).get(key, ""),
                "TR": _DATA.get("TR", {}).get(key, ""),
                "VI": _DATA.get("VI", {}).get(key, ""),
            }
...
async def dowload(path=None, lang: list = []):
    global _PATH
    ...
    if lang == []:
        ...
        correct_lang = []
        for key in lang:
            if key.upper() in LANGKEY:
                correct_lang.append(key.upper())
            else:
                print(f"Incorrect language code: {key}")
